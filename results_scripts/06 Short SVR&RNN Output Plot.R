# RESULTS : Short SVR Model Output Stats and Plots
...
for (i in loop) {
  ...
  eval(parse(text=paste0("preds",i," <- read.csv('results_data/super_short_",i,"_SVR_preds.csv')")))
  eval(parse(text=paste0("preds",i,"$chunk <- as.POSIXct(preds",i,"$chunk)")))
}
...
for (i in loop){
  ...
  eval(parse(text=paste0("ggsave(filename = 'results_output/super_short_",i,"_SVR_preds_plot.png', plot = plot, width = 3, height = 5)")))
  print(errors)
}
...
for (i in loop) {
  # read in correct data: train and test
  eval(parse(text=paste0("preds",i," <- read.csv('results_data/mo_short_",i,"_RNN_preds.csv')")))
  eval(parse(text=paste0("date <- read.csv('results_data/mo_short_",i,".csv') %>% dplyr::filter(traintest == 1) %>% dplyr::select(chunk, texts)")))
  eval(parse(text=paste0("preds",i," <- cbind(date[1:12,], preds",i,")")))
  rm(date)
}

preds1 # yep!
preds2 # this one's good
preds3 # yep!
preds4 #yep!!
preds5 # great!
preds6 # yes!!!!
...
for (i in loop){
  ...
  plot <- ggplot(preds, aes(x=localtime, y=truth, col="Truth")) + geom_line(alpha = 1) +
    ...
    ylim(30,350) #+ annotate("text", x=16, y = 200, label = "italic(MAE) == whatever",parse = TRUE)
  # + Choose colour blind colour scale!
  eval(parse(text=paste0("ggsave(filename = 'results_output/short_",i,"_RNN_preds_plot.png', plot = plot, width = 3, height = 5)")))
  print(errors)
}
